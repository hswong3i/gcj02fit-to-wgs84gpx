...
const dom = {
  ...
  statActivityDate: document.getElementById('statActivityDate'),
...
const transformLat = (lng, lat) => {
  ...
  ret +=
    ...
    3.0;
  ret +=
    ((160.0 * Math.sin((lat * Math.PI) / 12.0) +
...
const transformLng = (lng, lat) => {
  ...
  ret +=
    ...
    3.0;
  ret +=
    ((150.0 * Math.sin((lng * Math.PI) / 12.0) +
      300.0 * Math.sin((lng * Math.PI) / 30.0)) *
      2.0) /
    3.0;
  return ret;
};
...
const toWgs84 = (lng, lat, fromEncoding) => {
  if (fromEncoding === 'GCJ02') {
    return gcj02ToWgs84(lng, lat);
  }
  if (fromEncoding === 'BD09') {
...
const parseGpxToRecords = (gpxText) => {
  ...
  if (trkpts.length === 0) {
    throw new Error('No GPX track segments located.');
  }
  ...
  Array.from(trkpts).forEach((pt) => {
    ...
    const cadNode =
      pt.getElementsByTagName('cad')[0] ||
      pt.getElementsByTagName('gpxtpx:cad')[0];
    const pwrNode = pt.getElementsByTagName('power')[0];
    const tmpNode =
      pt.getElementsByTagName('atemp')[0] ||
      pt.getElementsByTagName('gpxtpx:atemp')[0];
    const spdNode =
      pt.getElementsByTagName('speed')[0] ||
      pt.getElementsByTagName('gpxtpx:speed')[0];
...
const parseTcxToRecords = (tcxText) => {
  ...
  if (trackpoints.length === 0) {
    throw new Error('No TCX track points located.');
  }
...
const generateFitOutput = (inputEncoding, outputEncoding, latLngs) => {
  ...
  parsedRecords.forEach((rec) => {
    if (!rec.position_long || !rec.position_lat) {
      return;
    }
    const [wgsLng, wgsLat] = toWgs84(
      rec.position_long,
...
const processAndRenderTrack = () => {
  ...
  if (parsedRecords[0]?.timestamp) {
    ...
    dom.statActivityDate.innerText = `${dateObj.toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} ${dateObj.toLocaleTimeString()}`;
  }
  ...
  parsedRecords.forEach((record) => {
    metrics.speed.push(
      (record.speed !== undefined && record.speed !== null ? record.speed : 0) *
        3.6,
    );
    metrics.hr.push(
    ...
    metrics.cad.push(
      record.cadence !== undefined && record.cadence !== null
        ? record.cadence
        : 0,
    );
    metrics.pwr.push(
      record.power !== undefined && record.power !== null ? record.power : 0,
    );
    metrics.ele.push(
  ...
  if (outputFormat === 'FIT') {
    outputBlobData = generateFitOutput(inputEncoding, outputEncoding, latLngs);
  } else if (outputFormat === 'TCX') {
    outputBlobData = generateTcxOutput(inputEncoding, outputEncoding, latLngs);
  } else {
    outputBlobData = generateGpxOutput(
      inputEncoding,
      outputEncoding,
      metrics,
      latLngs,
  ...
  dom.statDistance.innerText = activitySummary.total_distance
    ? `${(activitySummary.total_distance / 1000).toFixed(2)} km`
    : '0.00 km';
  ...
  dom.statElevation.innerText = activitySummary.total_ascent
    ? `${activitySummary.total_ascent.toFixed(0)} m`
    : '0 m';

  const calcAvgMax = (arr, round = 0) =>
    arr.length
      ? [
          (arr.reduce((a, b) => a + b, 0) / arr.length).toFixed(round),
  ...
  dom.statTemperature.innerText = metrics.temp.length
    ? `${avgTmp} / ${maxTmp} °C`
    : '-';
  ...
  chartInstances.speed = createTelemetryChart(
    'speedChart',
    'Speed',
    metrics.speed,
    '#0D6EFD',
  ...
  chartInstances.heartRate = createTelemetryChart(
    'heartRateChart',
    'Heart Rate',
    metrics.hr,
    '#E83E8C',
  ...
  chartInstances.cadence = createTelemetryChart(
    'cadenceChart',
    'Cadence',
    metrics.cad,
    '#198754',
  ...
  chartInstances.power = createTelemetryChart(
    'powerChart',
    'Power',
    metrics.pwr,
    '#DC3545',
  ...
  chartInstances.elevation = createTelemetryChart(
    'elevationChart',
    'Elevation',
    metrics.ele,
    '#6F42C1',
  ...
  chartInstances.temperature = createTelemetryChart(
    'temperatureChart',
    'Temperature',
    metrics.temp,
    '#FD7E14',
  ...
  if (latLngs.length > 0) {
    const polyline = L.polyline(latLngs, { color: '#0056B3', weight: 4 }).addTo(
      map,
    );
    ...
    const startIcon = L.divIcon({
      ...
      html: '<i class="bi bi-geo-alt-fill text-success fs-1"></i>',
      iconSize: [48, 48],
      iconAnchor: [24, 48],
    });
    ...
    L.marker(latLngs[latLngs.length - 1], { icon: endIcon })
      ...
      .bindPopup('End Point');
    showStatus(
      `Successfully loaded ${parsedRecords.length} records!`,
      'text-success',
    );
    dom.exportBtn.disabled = false;
  } else {
    showStatus(
      'Parsed track data successfully, but found no valid coordinates.',
      'text-warning',
    );
...
const parseAndProcess = async () => {
  ...
  try {
    if (inputFormat === 'FIT') {
      ...
      if (!decoder.isFIT()) {
        throw new Error(
          'Invalid file structure: Not a valid Garmin FIT target.',
        );
      }
      ...
      const rawRecords =
        messages?.record || messages?.records || messages?.recordMesgs || [];

      if (rawRecords.length > 0) {
        parsedRecords = rawRecords.map((rec) => ({
          position_lat: rec.positionLat
            ? rec.positionLat * (180 / 2147483648)
            : undefined,
          position_long: rec.positionLong
            ? rec.positionLong * (180 / 2147483648)
            : undefined,
          altitude:
            rec.enhancedAltitude !== undefined
          ...
          temperature: rec.temperature,
          speed:
            rec.enhancedSpeed !== undefined ? rec.enhancedSpeed : rec.speed,
          ...rec,
        }));
      }
      const sessions =
        messages?.session || messages?.sessions || messages?.sessionMesgs || [];
      if (sessions[0]) {
