...
const processAndRenderTrack = () => {
    ...
    if (outputFormat === "FIT") {
        ...
        outputBlobData = generateGpxOutput(inputEncoding, outputEncoding, metrics, latLngs);
    }
    ...
    if (activitySummary.total_moving_time) {
        const s = Math.floor(activitySummary.total_moving_time % 60),
            m = Math.floor((activitySummary.total_moving_time / 60) % 60),
            h = Math.floor(activitySummary.total_moving_time / 3600);
...
const parseAndProcess = async () => {
    ...
    try {
        if (inputFormat === "FIT") {
            ...
            if (!decoder.isFIT()) {
                throw new Error("Invalid file structure: Not a valid Garmin FIT target.");
            }
